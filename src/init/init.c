...
static void	new_img(t_game *game, t_img *img, int width, int height)
{
...
static void	load_one(t_game *game, t_img *tex, char *path)
{
	int	w;
	int	h;
	...
	if (!tex->img)
		parse_error(game, "failed to load texture (check the path in NO/SO/WE/EA)");
	tex->width = w;
	tex->height = h;
